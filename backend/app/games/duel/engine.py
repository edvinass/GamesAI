...
POWERUP_TYPES = ("rapid_fire", "shield", "wide_shot", "ghost")
...
class DuelEngine(GamePlugin):
    ...
    def _apply_ai_inputs(self, state: dict) -> None:
        ...
        for player in state["players"]:
            ...
            if not rethink:
                continue
            move, shoot, charge_start, charge_release, charge_ticks = choose_ai_actions(
                state, pid, fighter
            )
    ...
    def _try_shoot(self, state: dict, fighter: dict, owner_id: str) -> None:
        ...
        if fighter.get("effects", {}).get("wide_shot"):
            width = max(width, 3)
            fighter["effects"]["wide_shot"] = False
